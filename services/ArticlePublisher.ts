...
class ArticlePublisher {
  ...
  static ARTICLE_DIST_PATH: string = path.join(
    __dirname,
    "../app/public/article"
  );

  // A path of the article template file.
  static ARTICLE_TEMPLATE: Buffer = fs.readFileSync(
    path.join(__dirname, "../app/templates/article.ejs")
  );
  ...
  static md: MarkdownIt = new MarkdownIt({
    ...
    highlight: (str, language) => {
      ...
      return `<pre class="hljs"><code>${ArticlePublisher.md.utils.escapeHtml(
        str
      )}</code></pre>`;
    },
  })
    ...
    .use(mdContainer, "toggle", {
      ...
      render(tokens, idx) {
        ...
        if (tokens[idx].nesting === 1) {
          return `<details><summary>${ArticlePublisher.md.utils.escapeHtml(
            content[1]
          )}</summary>\n`;
        }
        return "</details>\n";
  ...
  private static getArticleByFilename(filename: string) {
    const mdContent = String(
      fs.readFileSync(`${this.ARTICLE_ORIGIN_PATH}/${filename}`)
    );
    const mdContentWithToc = `::: toggle(Table of Contents)\n[[toc]]\n:::\n${mdContent}`;
    const htmlContent: string = this.md.render(
      this.extractContent(mdContentWithToc)
    );
  ...
  public static extractMetaInfo(text: string): ArticleMetaInfo {
    ...
    const metaInfoLines: string[] = text
      .match(/(-{3})([\s\S]+?)(\1)/)[2]
      .match(/[^\r\n]+/g);
  ...
  public static publishArticles(id?: number) {
    const articleFiles: string[] = fs
      .readdirSync(this.ARTICLE_ORIGIN_PATH)
      .filter((file) => !this.IGNORED_FILES.includes(file));

    const distArticles: ArticleModel[] = articleFiles.map(
      (articleFile: string, index: number) => {
        const article =
          ArticlePublisher.getArticleByFilename(articleFile).getArticle();
        const nextArticle =
          articleFiles[index + 1] &&
          ArticlePublisher.getArticleByFilename(
            articleFiles[index + 1]
          ).getArticle();
        const prevArticle =
          articleFiles[index - 1] &&
    ...
    const sortedDistArticles: ArticleModel[] = distArticles.sort(
      (a, b) => a.id - b.id
    );
